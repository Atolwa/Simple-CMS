         ...
         <div class="row">
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="group_id" required>
                    ...
                    <option value="{{ $group->id }}" {{ $group->id === $contact->group ? 'selected' : '' }}>{{ $group->name }}</option>
                        @if ($group->children)
                            @foreach ($group->children as $child)
                            <option value="{{ $child->id }}" {{ $child->id === $contact->group_id ? 'selected' : '' }}>&nbsp;&nbsp;{{ $child->name }}</option>                            @endforeach
                        @endif
                    @endforeach
                </select>
            </div>
